...
- `.avqn/PRODUCT.md` — vision et descriptif fonctionnel du produit (rempli par le client au démarrage)
- `.avqn/STACK.md` — détails techniques de la stack
- `specs/` — un fichier `.md` par feature (géré par ce skill)
- `.avqn/DECISIONS.md` — journal des décisions techniques (géré par ce skill)

**Avant toute action**, lire `AGENTS.md` et `.avqn/PRODUCT.md` pour avoir le contexte projet. Si l'un de ces fichiers est absent ou vide, le signaler à l'utilisateur avant de continuer.
...
3. Si correctif rapide : faire la modification, lancer lint, commit avec préfixe `fix:`, push. Ne pas créer de fiche dans `specs/` mais ajouter une entrée dans `.avqn/DECISIONS.md`.
...
- `/ship-feature` → complète la partie 3 de la fiche, ajoute une entrée à `.avqn/DECISIONS.md`, commit + push

Ne jamais modifier `.avqn/PRODUCT.md`, `AGENTS.md` ou `.avqn/STACK.md` sans demander explicitement à l'utilisateur.
